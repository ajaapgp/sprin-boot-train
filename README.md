...
* テーブルは1テーブル
* 登録機能と検索機能と削除機能を実装する
* 照会機能と更新機能は未実装のため、あとで実装にチャレンジするべし
...
* 管理者(rootユーザでシステムロール保持)しか削除機能を使用することができないように認可を設定
  - `WebSecurityConfig`クラスの`.mvcMatchers("/WBA0401/**").hasRole("SYSTEM")`で実現している
...
| 登録画面 | 一般（user）の利用可 |
| 検索画面 | 一般（user）の利用可 |
| 詳細画面 `未実装` | 一般（user）の利用可 |
| 更新画面 `未実装` | 一般（user）の利用可 |
